...
$("html").on("paste", function(event) {
    if (event.target.id === 'customMaskURL' || event.target.id === 'saveFromURLURL' || event.target.id == "copyToClipboard") {} else if (event.target.id === 'mobileRISURL') {
...
function loadMask(selectedMask, alphaValue, origin, zoom, deform) {
    ...
    new fabric.Image.fromURL(url, function(mask) {
        ...
        if (deform != false) {
            ...
            maskImage.set({
                transformMatrix: [1, (Math.random() / 5) - 0.1, (Math.random() / 5) - 0.1, 1, 0, 0]
            });
        }
